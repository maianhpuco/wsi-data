...
def generate_tcga_splits(config):
    ...
    lusc_path = config['paths']['metadata']['lusc']
    ...
    print(f"Found {len(all_pt_files)} total .pt files across LUAD, LUSC")

    luad_df = pd.read_excel(luad_path)
    luad_df['label'] = 'LUAD'
    lusc_df = pd.read_excel(lusc_path)
    lusc_df['label'] = 'LUSC'


    df = pd.concat([luad_df, lusc_df], ignore_index=True)

...
def seg_and_patch(source, annotation_dir, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir, 
                  patch_size=256, step_size=256, 
                  seg_params={'seg_level': -1, 'sthresh': 8, 'mthresh': 7, 'close': 4, 'use_otsu': False,
                              'keep_ids': 'none', 'exclude_ids': 'none'},
                  filter_params={'a_t': 100, 'a_h': 16, 'max_n_holes': 8}, 
                  vis_params={'vis_level': -1, 'line_thickness': 500},
                  patch_params={'use_padding': True, 'contour_fn': 'four_pt', 'white_thresh': 5, 'black_thresh': 50},
                  patch_level=0,
    ...
    for i in tqdm(range(total)):
        ...
        mask_file = os.path.join(annotation_dir, slide_id + '_mask.tiff')
        ...
        seg_time_elapsed = -1
        if seg:
            WSI_object, seg_time_elapsed = segment(WSI_object, current_seg_params, current_filter_params, mask_file if os.path.exists(mask_file) else None) 
...
if __name__ == "__main__":
    ...
    for split in ['train', 'test', 'validation']:
        ...
        annotation_dir = paths['annotation_dir'][split]

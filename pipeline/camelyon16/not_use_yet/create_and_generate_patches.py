...
def patch_and_save(wsi_obj, patch_params, slide_id, patch_save_dir, patch_png_dir):
    patch_path = os.path.join(patch_save_dir, f"{slide_id}.h5")
    png_dir = os.path.join(patch_png_dir, slide_id)
    ...
    for cont_idx, cont in enumerate(wsi_obj.contours_tissue):
        ...
        for patch_info in patch_generator:
            ...
            patch.save(os.path.join(png_dir, filename))
            if first_patch:
                patch_path = wsi_obj.createPatches_bag_hdf5(
                    save_path=patch_save_dir,
                    patch_level=patch_params['patch_level'],
                    patch_size=patch_params['patch_size'],
                    step_size=patch_params['step_size'],
...
def seg_and_patch(config):
    ...
    patch_save_dir = paths['patch_h5_dir']
    ...
    for idx in process_stack.index:
        ...
        if auto_skip and os.path.exists(os.path.join(patch_png_dir, slide_id)) and os.path.exists(os.path.join(patch_save_dir, slide_id + '.h5')):
        ...
        if patch:
            ...
            patch_and_save(wsi_obj, {
                ...
                'black_thresh': patch_params['black_thresh'],
                'contour_fn': patch_params['contour_fn']
            }, slide_id, patch_save_dir, patch_png_dir)
            patch_times += time.time() - start
            print(f"[INFO] PNG + H5 Patching took {patch_times:.2f}s")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Segmentation + patching to PNG and H5')

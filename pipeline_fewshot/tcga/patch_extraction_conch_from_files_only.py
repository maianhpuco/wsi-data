...
def main(args):
    ...
    if not os.path.exists(args.missing_file):
        print(f"[✓] No missing file found at {args.missing_file}. Finish.")
        return

    missing_df = pd.read_csv(args.missing_file)
    slides_to_process = missing_df['slide_id'].unique().tolist()

    processed, skipped = 0, 0

    for slide in tqdm(slides_to_process):
        slide_path = os.path.join(args.patches_path, slide)
        out_path = os.path.join(args.conch_features_path, slide)

        if not os.path.isdir(slide_path):
            print(f"[WARN] Folder not found for slide: {slide_path}")
            skipped += 1
            continue
    ...
    print("\n[✓] Extraction summary")
    print(f"Total missing entries in file: {len(slides_to_process)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--missing_file', type=str, required=True, help="Path to CSV file containing missing slide IDs")

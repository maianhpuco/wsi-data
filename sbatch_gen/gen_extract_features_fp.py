...
def get_script_path(dataset):
    if dataset == "camelyon16":
        return "pipeline_fewshot/camelyon16/extract_conch_fp.py"
    else:
        return "pipeline_fewshot/tcga/extract_conch_fp.py"
...
for idx, dataset in enumerate(datasets):
    for mag in magnifications:
        job_name = f"ec{mag}_{dataset}"
        log_name = f"extract_conch_fp_{mag}_{dataset}.log"
        sbatch_name = f"extract_conch_fp_{mag}_{dataset}.sbatch"
